...
export const getInwards = async () => {
  await delay(300);
  return JSON.parse(localStorage.getItem("inwards")) || [];
};
...
export const saveInward = async (inward) => {
  ...
  // 🔒 prevent duplicate inward numbers
  if (inwards.some((i) => i.inwardNo === inward.inwardNo)) {
    throw new Error("Inward number already exists");
  }
