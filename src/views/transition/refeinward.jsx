...
const InwardPage = () => {
  ...
  // ✅ Load saved inwards on mount
  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem("inwards")) || [];
    setInwards(saved);
  }, []);
  ...
  const handleSave = () => {
    ...
    const updatedInwards = [...inwards, newInward];
    setInwards(updatedInwards);
    localStorage.setItem("inwards", JSON.stringify(updatedInwards));
            ...
            {inwards.length > 0 ? (
              inwards.map((inv, idx) => (
                <tr key={idx}>
                  ...
                  <td>{inv.referenceNo}</td>
                  <td>{inv.materials.length}</td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan="5" className="text-center text-muted">
                  No inwards saved yet.
                </td>
              </tr>
            )}
          </tbody>
